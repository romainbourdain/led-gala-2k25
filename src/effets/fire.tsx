...
export const updateFireEffect: EffectUpdateCanvas<{
  ...
  const particles = createFireParticles(100, canvas.width, canvas.height);
  ...
  const animateFire = () => {
    ...
    particles.forEach((particle) => {
      ...
      gradient.addColorStop(0, "rgba(255, 140, 0, " + particle.opacity + ")");
      gradient.addColorStop(
        0.5,
        "rgba(255, 69, 0, " + particle.opacity * 0.8 + ")"
      );
      ...
      if (particle.y < canvas.height - params.height || particle.opacity <= 0) {
        particle.x = canvas.width / 2 + (Math.random() - 0.5) * 50;
